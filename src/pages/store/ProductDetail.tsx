...
export function ProductDetail() {
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-[#FFFFFF]">
        <Navbar />
        <div className="flex flex-col items-center justify-center h-96 text-[#000000]/40 gap-4">
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs sm:text-sm text-[#000000]/50 mb-6">
          ...
          <span>/</span>
          <span className="text-[#000000] truncate max-w-[200px]">{product.name}</span>
        </nav>
          ...
          <div className="lg:col-span-7">
            <div className="grid grid-cols-12 gap-3">
                  ...
                  <div className="flex sm:flex-col gap-2 sm:max-h-[560px] overflow-x-auto sm:overflow-y-auto">
                    {images.map((url, i) => (
                      <button
                        ...
                        className={`flex-shrink-0 w-16 h-16 sm:w-full sm:h-20 rounded-xl overflow-hidden bg-[#F5F5F5] border-2 transition-all ${
                          activeImage === i
                            ? 'border-[#E63939] ring-2 ring-[#E63939]/20'
                            : 'border-[#E5E7EB] hover:border-[#E63939]/50'
                        }`}
              ...
              <div className={`col-span-12 ${images.length > 1 ? 'sm:col-span-10' : ''} order-1 sm:order-2`}>
                <div className="relative aspect-square bg-[#FFFFFF] rounded-xl overflow-hidden border border-[#E5E7EB] group">
                  ...
                  {product.featured && (
                    <span className="absolute top-4 left-4 bg-[#E63939] text-white text-xs font-semibold px-3 py-1 rounded-md uppercase tracking-wider">
                      Best Seller
                    </span>
                  )}
                </div>
          ...
          <div className="lg:col-span-5">
            {product.categories && (
              <Link
                ...
                className="text-xs font-semibold text-[#E63939] uppercase tracking-widest hover:underline"
              >
                {product.categories.name}
              </Link>
            )}
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#000000] mt-2 mb-4 leading-[1.15] text-balance">
              {name}
            </h1>
            ...
            <div className="flex items-center gap-4 mb-5 flex-wrap">
              <div className="flex items-center gap-1">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-4 h-4 ${i < 4 ? 'fill-amber-400 text-amber-400' : 'text-[#E5E7EB]'}`}
                  />
                ))}
                <span className="text-xs text-[#000000]/50 ml-1.5 font-medium">4.0 (24 reviews)</span>
              </div>
                ...
                {product.variants.map((variant) => {
                  ...
                  return (
                    <div key={variant.name}>
                      <p className="text-xs font-semibold text-[#000000]/50 uppercase tracking-widest mb-2">
                        {variant.name}
                        {selectedVariants[variant.name] && (
                          <span className="ml-2 text-[#000000] normal-case font-semibold tracking-normal">
                            — {selectedVariants[variant.name]}
                          </span>
                        )}
                      </p>
                            ...
                            return (
                              <button
                                ...
                                className={`w-8 h-8 rounded-full border-2 transition-all ${
                                  active ? 'border-[#E63939] scale-110 ring-2 ring-[#E63939]/30' : 'border-[#E5E7EB] hover:border-[#E63939]/50'
                                }`}
                          ...
                          return (
                            <button
                              ...
                              className={`px-3 py-1.5 text-sm font-semibold rounded-lg border-2 transition-all ${
                                active
                                  ? 'border-[#E63939] bg-[#E63939]/10 text-[#E63939]'
                                  : 'border-[#E5E7EB] text-[#000000] hover:border-[#E63939]/50'
                              }`}
                            >
                              {opt}
                            </button>
            ...
            {!isOutOfStock && (
              <div className="flex items-stretch gap-3 mb-3">
                <div className="flex items-center bg-[#F5F5F5] border border-[#E5E7EB] rounded-lg overflow-hidden">
                  <button
                    onClick={() => setQty((q) => Math.max(1, q - 1))}
                    className="px-3 sm:px-4 py-3 hover:bg-[#E5E7EB] text-[#000000]/50 transition-colors"
                    aria-label="Decrease quantity"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="px-4 font-bold text-[#000000] min-w-[2.5rem] text-center">{qty}</span>
                  <button
                ...
                <motion.button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 bg-[#E63939] hover:bg-[#C82020] text-white font-bold py-3 rounded-lg transition-all text-sm sm:text-base"
                >
            ...
            {isOutOfStock && (
              <div className="py-3 px-4 bg-red-100 text-red-700 rounded-lg text-sm font-semibold mb-3 text-center border border-red-200">
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border-2 font-semibold text-sm transition-all mb-3 ${
                inWishlist
                  ? 'border-[#E63939] bg-[#E63939]/10 text-[#E63939]'
                  : 'border-[#E5E7EB] text-[#000000]/50 hover:border-[#E63939]/50'
              }`}
            >
            ...
            <div className="grid grid-cols-2 gap-3 pt-5 border-t border-[#E5E7EB]">
              {[
                ...
                <div key={label} className="flex items-center gap-2.5">
                  <div className="w-9 h-9 bg-[#E63939]/10 rounded-lg flex items-center justify-center flex-shrink-0 border border-[#E63939]/20">
                    <Icon className="w-4 h-4 text-[#E63939]" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-[#000000] leading-tight">{label}</p>
                    <p className="text-[11px] text-[#000000]/50">{sub}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mt-12 lg:mt-16 bg-[#F9FAFB] rounded-xl border border-[#E5E7EB] overflow-hidden">
          <div className="border-b border-[#E5E7EB] flex bg-white">
            {([
              ...
              <button
                ...
                className={`px-6 py-4 text-sm font-semibold transition-colors relative ${
                  tab === key ? 'text-[#E63939]' : 'text-[#000000]/50 hover:text-[#000000]'
                }`}
              >
                {label}
                {tab === key && (
          ...
          <div className="p-6 sm:p-8 bg-white">
            {tab === 'description' ? (
              <div className="prose prose-sm max-w-none text-[#000000]/70 leading-relaxed">
                {description ? (
                  <p>{description}</p>
                ) : (
                  <p className="text-[#000000]/40 italic">No description available for this product yet.</p>
                )}
              </div>
            ) : (
              <table className="w-full">
                <tbody>
                  {specs.map((row, i) => (
                    <tr key={row.label} className={i % 2 === 0 ? 'bg-[#F5F5F5]' : ''}>
                      <td className="px-4 py-3 text-sm font-semibold text-[#000000] w-1/3">{row.label}</td>
                      <td className="px-4 py-3 text-sm text-[#000000]/70">{row.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-12 lg:mt-16">
            <div className="flex items-end justify-between mb-6">
              <div>
                <p className="text-xs font-semibold text-[#E63939] uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                  <Zap className="w-3 h-3 fill-[#E63939]" />
                  You might also like
                </p>
                <h2 className="text-2xl sm:text-3xl font-bold text-[#000000]">Related Products</h2>
              </div>
              <Link
                to={`/shop${product.categories ? `?category=${product.categories.slug}` : ''}`}
                className="hidden sm:inline-flex items-center gap-1 text-sm font-semibold text-[#E63939] hover:gap-2 transition-all"
              >
